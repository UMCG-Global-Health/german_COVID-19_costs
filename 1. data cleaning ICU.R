...
colnames(df)[22] <- "Outcome" #1 is death


df <- df %>%  
  filter(ICU.length.of.stay.hours > 0) %>% 
  filter(Age>17)
...
df$Admission_Month <- as.factor(df$Admission_Month)
...
#Nas introduced by coercion. Need to adjust the excel file (change characters to numbers see google)
...
df <- df %>% 
  mutate(ECMO_dur_days = ECMO_dur/24 )
df$los_ICU_non_mech[df$los_ICU_non_mech<0] <- 0


skim(df)

write.csv(df, "./data/GUF_cleaned_ICU.csv")
